...
export interface HtmlCleanOptions {
  /** 要保留的標籤 (預設保留基本結構標籤) */
  keepTags?: string[];
  /** 要移除的標籤 (預設移除 script, style, svg 等) */
  removeTags?: string[];
  /** 是否保留圖片連結 */
  keepImages?: boolean;
  /** 最大內容長度限制 */
 ...
 * 預設清理選項
 */
const DEFAULT_OPTIONS: Required<HtmlCleanOptions> = {
 ...
 */
export function htmlToMarkdown(html: string, options: HtmlCleanOptions = {}): string {
 ...
 * 移除不需要的標籤
 */
function removeUnwantedTags(html: string, removeTags: string[]): string {
  let result = html;
 ...
 * 保留圖片連結
 */
function preserveImageLinks(html: string): string {
 ...
 * 將 HTML 轉換為 Markdown
 */
function convertToMarkdown(html: string, keepTags: string[]): string {
  let result = html;

  // 轉換標題
  result = result.replace(/<h([1-6])[^>]*>(.*?)<\/h\1>/gi, (_, level: string, content: string) => {
    const cleanContent = cleanHtmlTags(content);
    return '#'.repeat(parseInt(level)) + ' ' + cleanContent + '\n\n';
  });

  // 轉換段落
  result = result.replace(/<p[^>]*>(.*?)<\/p>/gi, (_: string, content: string) => {
    const cleanContent = cleanHtmlTags(content);
    return cleanContent + '\n\n';
  });
  ...
  result = result.replace(/<ul[^>]*>(.*?)<\/ul>/gi, (_, content) => {
    const listItems = content.replace(/<li[^>]*>(.*?)<\/li>/gi, (_: string, item: string) => {
